...
{
srand(time(NULL));
int czyszczenie=1,a,pktg1=0,pktg2=0,muzyka=1,u=0,fullscreen=1,pwrup1,pwrup2,pwrup3=1,pwrup4=0,pwrup5=0;
float q=0.00005,stangry[5][10],stangry2[20];
const sf::Vector2i okno(1440,900); // wektor wymiarow okna
sf::RenderWindow oknogry(sf::VideoMode(okno.x,okno.y), "Pong by Aleksander Makowka",sf::Style::Fullscreen); //sf::Style::Fullscreen
...
textc.setColor(sf::Color::Red);
textc.setStyle(sf::Text::Regular);
textc.setPosition(sf::Vector2f(okno.x-300,0));

//gracz 1
sf::RectangleShape gracz1;
gracz1.setSize(sf::Vector2f(10,200));
gracz1.setFillColor(sf::Color(0,0,255));
gracz1.setPosition(sf::Vector2f(0,(okno.y/2)-100));

//gracz 2
sf::RectangleShape gracz2;
gracz2.setSize(sf::Vector2f(10,200));
gracz2.setFillColor(sf::Color(255,0,0));
gracz2.setPosition(sf::Vector2f(okno.x-10,(okno.y/2)-100));

//pilka
sf::CircleShape pilka;
pilka.setRadius(20);
pilka.setFillColor(sf::Color(0,255,0));
pilka.setOrigin(20,20);
sf::Vector2f pb((okno.x/2),(okno.y/2)); // wektor polozenia bazowego pilki
sf::Vector2f v(0.15,0.15); // wektor predkosci pilki
...
ssn<<"Gracz niebieski: "<<pktg1<<endl;
textn.setString(ssn.str());

ostringstream ssc;
ssc<<"Gracz czerwony: "<<pktg2<<endl;
...
for(int i=0;i<20;i++) //////////// petla gry gracz vs gracz
{
if(v.x<0 && q<0)// predkosc v.x + wartosc q. ustawianie q zgodnie z v.x
{
v.x=v.x+q;
}
if(v.x<0 && q>0)
...
fout<<stangry[0][0]<<endl<<stangry[0][1]<<endl<<stangry[0][2]<<endl<<stangry[0][3]<<endl;
fout<<stangry[1][0]<<endl<<stangry[1][1]<<endl;
fout<<stangry[2][0]<<endl<<stangry[2][1]<<endl;
...
sf::Vector2f pz=pilka.getPosition(); // wektor pozycji pilki
sf::Vector2f pozgracz2=gracz2.getPosition(); // wektor pozycji gracza2
// ruch w dol
if(v.y>=0)
{
if (pozgracz2.y < pz.y-60)
{
if (pozgracz2.y <=okno.y-200)
{
    gracz2.move(0, 8);
...
pktg1=stangry2[5];
gracz2.setPosition(okno.x-10,stangry2[6]);
pktg2=stangry2[7];
u=stangry2[8];
